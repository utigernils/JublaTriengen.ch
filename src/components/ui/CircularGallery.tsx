import { Camera, Color, Mesh, Plane, Program, Renderer, Texture, Transform, Raycast, Vec2 } from 'ogl';
...
class App {
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    if (this.onClickCallback) {
      ...
      const dx = clientX - this.clickStart.x;
      const dy = clientY - this.clickStart.y;
  ...
  handleClick(clientX: number, clientY: number) {
    ...
    // Make sure to account for the canvas position within the viewport
    ...
    // We need to test against the planes in our medias
    ...
    if (hits && hits.length) {
      ...
      const mediaIndex = this.medias.findIndex(media => media.plane === hitMesh);
      
      if (mediaIndex !== -1 && this.mediasImages[mediaIndex]) {
        // Since we duplicated the items in createMedias (concatenation),
        // we need to return the original index relative to the passed items.
        // The original length is kept in this.mediasImages.length / 2, or just pass the mod index
        // But let's check how mediasImages is constructed.
        // It is galleryItems.concat(galleryItems).
        
        // However, the user passed 'items'.
        // Let's see if we can derive the original index.
        const originalLength = this.mediasImages.length / 2;
        const realIndex = mediaIndex % originalLength;
        this.onClickCallback!(realIndex);
      }
    }
  }
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, backgroundColor, borderRadius, font, scrollSpeed, scrollEase, onClick]);
